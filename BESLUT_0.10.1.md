# BESLUT 0.10.1 — fas B: egen datapipeline mot intervals.icu

**Datum:** 2026-08-05 · **Från:** 0.9.4 · **Tester:** 517 kärna (från 366) + 210 rök (från 139)
...
## 0.10.1 — leveransvakt efter verklig incident

**Incident 2026-08-05.** 0.10.0 levererades två gånger med olika innehåll och **samma stämpel**: först utan `swimHrValid`, sedan med. `sw.js` cachar `ui.js` och `core.js` cache-first, och cachenamnet bärs av stämpeln. Service workern hade därför ingen anledning att invalidera något — produktägaren deployade den andra leveransen och fick den förstas `ui.js`. `index.html` är network-first och visade rätt stämpel, vilket dolde felet: paritetskortet såg grönt ut medan koden var blandad.

**Rotorsak:** stämpeln behandlades som ett versionsnummer i stället för som en cachenyckel. Två kodläge med samma stämpel är precis det paritetskortet finns för att omöjliggöra — samma felklass som sessionen tidigare identifierat i arbetsfilerna, den här gången orsakad av utvecklaren.

**Regel som följer:** **varje zip får eget patchnummer.** Utan undantag, även för en enradsfix inom samma session. En stämpel är förbrukad i samma sekund den lämnat verkstaden.

**Vakt i kod:** tre röktester låser nu att `core.js` och `ui.js` bär samma stämpel, och att `sw.js`-cachenamnet följer den. Ett kodläge som inte kan invalidera sin egen cache blir rött i sviten.
